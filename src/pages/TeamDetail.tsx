...
// --- KOMPONEN KARTU PEMAIN (TOKEN) ---
const PlayerToken = ({ player, onDelete, isSub = false }: { player: Player; onDelete: (id: number) => void, isSub?: boolean }) => {
  // Ukuran dinamis berdasarkan apakah dia pemain inti atau cadangan, dan ukuran layar
  ...
  <div className={`flex flex-col items-center group relative z-10 ${containerSize} flex-shrink-0`}>
    ...
    <div className="relative mb-1 transition-transform duration-300 group-hover:scale-110 cursor-pointer">
        ...
        {/* Nomor Punggung (Badge Kecil) */}
        <div className={`absolute -bottom-1 -right-1 bg-pl-cyan text-pl-primary font-black ${badgeSize} flex items-center justify-center rounded-full border md:border-2 border-pl-primary shadow-sm`}>
            {player.number}
        </div>
        ...
        <button 
            ...
            className="absolute -top-2 -right-2 bg-pl-pink text-white p-0.5 md:p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-md hover:bg-red-600 z-20"
        >
...
export default function TeamDetail() {
  ...
  const showModal = (type: 'danger' | 'success' | 'info', title: string, message: string, onConfirm?: () => void) => {
    ...
    showModal(
      ...
      async () => {
        try {
          ...
          showModal('danger', 'Error', 'Failed to remove player.');
        }
      }
    );
  };

  // --- LOGIKA PEMISAHAN SQUAD ---
  ...
  if (loading) return <div className="min-h-screen bg-pl-dark flex justify-center items-center text-pl-cyan font-black tracking-widest animate-pulse">LOADING TACTICS...</div>;
  ...
  return (
    <div className="min-h-screen bg-pl-dark pt-24 md:pt-28 pb-32 md:pb-12 px-3 sm:px-6 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-[#2c0030] to-pl-dark overflow-x-hidden">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
            ...
            {/* Info Klub di Mobile (Pindah ke Atas) */}
            <div className="md:hidden flex items-center gap-3 bg-[#240026] p-3 rounded-2xl border border-white/5">
                <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm">
                    {team.logo ? <img src={team.logo} className="w-full h-full object-contain p-1"/> : <Shield className="text-gray-300 w-6 h-6"/>}
                </div>
                <div>
                    <h1 className="text-lg font-black text-white leading-none mb-1">{team.name}</h1>
                    <span className="text-[10px] font-bold text-pl-gray uppercase tracking-wider block">{team.coach}</span>
                </div>
            </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {/* KOLOM KIRI: PITCH VISUALIZATION (STARTING XI) */}
            ...
            {/* KOLOM KANAN: FORM & INFO (DI MOBILE JADI PALING BAWAH) */}
            <div className="lg:col-span-1 space-y-6 order-2">
                
                {/* Club Info Card (Desktop Only - Karena Mobile sudah di header atas) */}
                <div className="hidden md:flex bg-[#240026] p-5 md:p-6 rounded-3xl border border-white/5 items-center gap-4 md:gap-5 shadow-lg relative overflow-hidden group">
                    ...
                    <form onSubmit={handleAddPlayer} className="flex flex-col gap-3 md:gap-4">
                        <div>
                            ...
                            <input 
                                required 
                                type="number" 
                                className="w-full p-2.5 md:p-3 bg-black/30 border border-white/10 text-white rounded-xl focus:border-pl-cyan focus:ring-1 focus:ring-pl-cyan outline-none font-mono font-bold text-sm md:text-base placeholder-white/10 transition-all" 
                                placeholder="00"
                        ...
                        <div>
                            ...
                            <input 
                                required 
                                className="w-full p-2.5 md:p-3 bg-black/30 border border-white/10 text-white rounded-xl focus:border-pl-cyan focus:ring-1 focus:ring-pl-cyan outline-none font-bold text-sm md:text-base placeholder-white/10 transition-all" 
                            ...
                            <div className="grid grid-cols-4 gap-1.5 md:gap-2">
                                {['GK', 'DF', 'MF', 'FW'].map((pos) => (
                                    <button
                                        key={pos}
                                        type="button"
                                        onClick={() => setNewPlayer({...newPlayer, position: pos})}
                                        className={`py-2 md:py-2.5 rounded-lg text-[9px] md:text-[10px] font-black transition-all border uppercase tracking-wider ${
                                            newPlayer.position === pos 
                                            ? 'bg-pl-cyan text-pl-primary border-pl-cyan shadow-lg' 
                                            : 'bg-black/30 text-pl-gray border-white/5 hover:bg-white/5 hover:text-white'
                                        }`}
                                    >
                                        {pos}
                                    </button>
                        ...
                        <button 
                            type="submit" 
                            className="mt-2 bg-gradient-to-r from-pl-pink to-red-600 text-white py-3 md:py-3.5 rounded-xl font-black uppercase tracking-widest hover:scale-[1.02] active:scale-95 transition-all shadow-lg shadow-pl-pink/20 flex items-center justify-center gap-2 text-xs md:text-sm"
                        >
        ...
        <Modal 
          ...
          type={modal.type}
          confirmText={modal.type === 'danger' ? 'Remove' : 'OK'}
        />
      </div>
    </div>
  );
...
// Komponen kecil untuk placeholder jika posisi kosong di lapangan
const NoPlayerBadge = ({ pos }: { pos: string }) => (
    <div className="text-white/20 text-[9px] md:text-xs font-bold uppercase border border-white/10 px-2 py-1 md:px-3 rounded-full backdrop-blur-sm">
        No {pos}
    </div>
);